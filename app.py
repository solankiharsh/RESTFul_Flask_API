...
from flask_jwt_extended import JWTManager, jwt_required, create_refresh_token, create_access_token
...
def retrieve_password(email: str):
    ...
    if user:
        msg = Message("Your planetary API password is " + user.password,
                      sender="admin@harshdemo-api.com",
                      recipients=[email])
        mail.send(msg)
...
def add_planet():
    ...
    if test:
        return jsonify("There is a planet already by that name"), 409
    else:
...
def update_planet():
    ...
    if planet:
        ...
        db.session.commit()
        return jsonify(messag='you updated a planet'), 200
    else:
